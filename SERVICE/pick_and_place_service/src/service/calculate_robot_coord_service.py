...
class CalculateRobotCoordService(BaseService):
    ...
    def cal_robot_coord(self, image, pcb_width, pcb_height):
        ...
        angle = result.angle
        top_right_x = result.right_top_x
        top_right_y = result.right_top_y

        print(top_right_x, top_right_y)

        cx, cy, robot_angle = self.compute_robot_pose_from_top_right([top_right_x, top_right_y], angle, pcb_width, pcb_height)
        ...
        cv2.putText(image, f"Score: {result.score:.4f}", (draw_x, draw_Y-50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
        cv2.putText(image, f"Angle: {angle:.4f} -- X: {top_right_x:.4f} -- Y: {top_right_y:.4f}", (draw_x, draw_Y), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
